...
import createWrapperStyle from './util/createWrapperStyle'
...
export default class PopupStub extends Component {
  ...
  addPopup (element, option) {
    ...
    let opt = resetOptions(option)
    opt.animValue = new Animated.Value(0)
  ...
  removePopup (id, forceUpdate = true) {
    ...
    popup.animValue.setValue(100)
    Animated.timing(popup.animValue, {
      toValue: 0,        // Target
      duration: popup.duration,    // Configuration
    }).start(() => {
      this.removePopupImmediately(id)
...
function resetOptions (option) {
  ...
  let opt = Object.assign({
    id: uuidV1(), zIndex: 1, mask: true, duration: 1000, position: 'center'
  }, option)

  // reset wrapperStyle
  opt.wrapperStyle = createWrapperStyle(opt.wrapperStyle, opt.position)
...
function createPopupElements (popups, maskColor) {
  ...
  popups.forEach((popup, id) => {
    ...
    const {duration, animValue, onAnimationEnd = () => {}} = popup
    popupElements.push(
      <PopupWrapper
        key={id}
