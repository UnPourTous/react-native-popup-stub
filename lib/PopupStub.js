...
import { Dimensions, Platform, View, TouchableWithoutFeedback, StyleSheet } from 'react-native'
...
export default class PopupStub extends Component {
  ...
  */
  ...
  render () {
    ...
    const { maskColor, orientation } = this.props
    const fullStyle = styles[orientation]
    let rendered = []

    popups.forEach((popup) => {
      if (popup.visible) {
        rendered.push( createPopupElement(popup, maskColor, fullStyle) )
      }
    })
...
function createPopupElement (popup, maskColor, fullStyle) {
  ...
  return (
    ...
    >
      {popup.mask ? <TouchableWithoutFeedback onPress={() => onAutoClose(popup)}>
        <View style={[fullStyle, {backgroundColor: maskColor}]} />
